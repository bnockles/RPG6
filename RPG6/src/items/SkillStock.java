...
public class SkillStock {
	...
	public static ActiveSkill skillStock(int type) {
		if (type == FB){
			return new ActiveSkill("Fireblast","fire fixes everything",50,10);
		}
		if (type == MG){
			return new ActiveSkill("Mana Guard","shields are essential",50,10);
		}
		if (type == EB){
			return new ActiveSkill("Energy Bolt","Pure energy hurts",50,10);
		}
		if (type == ROI){
			return new ActiveSkill("Reign of Ice","Ice ice baby",50,10);
		}
		if (type == GCR){
			return new ActiveSkill("Gravity Core","control gravity",50,10);
		}
		if (type == EX){
			return new ActiveSkill("Explosion","more fire",50,10);
		}
		if (type == M){
			return new ActiveSkill("Meditate","too much causes hallucinations",50,10);
		}
		if (type == WN){
			return new ActiveSkill("Water Nebula","Agua Novela",50,10);
		}
		if (type == RS){
			return new ActiveSkill("Runesave","No mp to give",50,10);
		}
		if (type == FT){
			return new ActiveSkill("Forbidden Tomb","For the enemies you really hate",50,10);
		}
		if (type == RC){
			return new ActiveSkill("Rock Crush","Avalanche was not enough",50,10);
		}
		if (type == DR){
			return new ActiveSkill("Death Ray","why not",99999999,1000);
		}
		if (type == TS){
			return new ActiveSkill("Triple Shot","Third times the charm",50,10);
		}
		if (type == SS){
			return new ActiveSkill("Scatter Shot","to make sure enenmies can't escape",50,10);
		}
		if (type == R){
			return new ActiveSkill("Ricochet","fire",50,10);
		}
		if (type == PA){
			return new ActiveSkill("Poison Arrow","Poison a convenient answer",50,10);
		}
		if (type == SR){
			return new ActiveSkill("Speed Run","go fast",50,10);
		}
		if (type == ROF){
			return new ActiveSkill("Rain of Fire","even more fire",50,10);
		}
		if (type == PS){
			return new ActiveSkill("Paralyzing Shot","fire at will",50,10);
		}
		if (type == SI){
